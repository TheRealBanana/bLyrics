...
#
# Created: Mon Sep 29 19:21:53 2014
...
class Ui_customQueryDialog(object):
    def setupUi(self, customQueryDialog, storage):
        ...
        QtCore.QObject.connect(self.submitCancelbuttons, QtCore.SIGNAL(_fromUtf8("accepted()")), self.submitQuery)
    ...
    def retranslateUi(self, customQueryDialog):
        customQueryDialog.setWindowTitle(_translate("customQueryDialog", "", None))
    ...
    def submitQuery(self):
        artist = self.artistEntryText.text()
        song = self.songTitleEntryText.text()
        self.storage.append(artist)
        self.storage.append(song)
        self.opACCEPT()