...
class LyricsProvider(object):
    ...
    def getLyrics(self, song, artist):
        ...
        surl = "http://www.songlyrics.com/index.php?section=search&searchW=%s&submit=Search"
        ...
        try:
            ...
            #We now make one attempt to match up the song title with the first few results using difflib
        ...
        #Now we cut out just the lyrics, unfortunately they appeared to be garbled just like lyricswiki does to their songs
    ...
    def songlyrics_getLyricsURL(self, song, artist, html):
        ...
        for entry in split_results:
            if re.search("serpdesc-2", entry, re.I) is not None: #make sure this is a lyrics entry and not garbage
                ...
                #Then we retest against the self._MASTER_RATIO and see if it fits now. If not again we forget about it (this could be bad when trying to differentiate between two songs that only differ by their end)
